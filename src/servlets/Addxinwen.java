...
public class Addxinwen extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try{
		    ...
		    String xdate=req.getParameter("xdate");
		    String xintro=req.getParameter("xintro");
		    String xtag=req.getParameter("xtag");
			...
			//设置存到数据库中的路径
			String ximage=url+filename+"."+ext;
			int cmark=0;
			String sql="insert into xinwen(xid,biaoti,xdate,xintro,xtag,ximage) values (?,?,?,?,?,?)";
			DB db=new DB();
			conn=db.createConn();
			conn.setAutoCommit(true);
			pstmt=conn.prepareStatement(sql);
			pstmt.setString(1,xid);
			pstmt.setString(2,biaoti);
			pstmt.setString(3,xdate);
			pstmt.setString(4,xintro);
			pstmt.setString(5,xtag);
			pstmt.setString(6,ximage);
